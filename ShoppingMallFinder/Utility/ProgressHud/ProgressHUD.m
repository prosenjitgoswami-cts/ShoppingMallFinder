...
@property (atomic, MB_STRONG,readwrite) UIView *indicator;
...
+(ProgressHUD *)progressHUDAddedToView:(UIView *)view
{
    UIView *viewToRemove = nil;
    
	for (UIView *v in [view subviews])
    {
		if ([v isKindOfClass:[ProgressHUD class]])
        {
			viewToRemove = v;
    ...
    return (ProgressHUD *)viewToRemove;
}
...
+(ProgressHUD *)flashWithMessage:(NSString *)message onView:(UIView *)view forInterVal:(NSTimeInterval)interval
{
...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self)
    {
		...
		self.opacity = 0.8f;
        self.color = [UIColor colorWithWhite:0.0 alpha:.9];
		...
		self.square = NO;
       // self.hudSize = CGSizeMake(37.0, 37.0);
...
- (void)setupLabels
{
	...
	_label.textAlignment = MBLabelAlignmentCenter;
	_label.opaque = NO;
...
- (void)layoutSubviews {
    ...
    if(self.hudSize.width && self.hudSize.height)
    {
        indicatorF.size.width  = self.hudSize.width;
        indicatorF.size.height = self.hudSize.height;
    }
    
    
	//CGRect indicatorF = ([indicator isKindOfClass:[UIActivityIndicatorView class]]) ? indicator.bounds : CGRectMake(0,0,self.hudSize.width, self.hudSize.height);
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {	
    ...
    else if (orientation == UIDeviceOrientationPortrait || orientation == UIInterfaceOrientationPortraitUpsideDown)
    {
